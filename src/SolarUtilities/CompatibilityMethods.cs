...
namespace BuildingCoder
{
    ...
    public static class CompatibilityMethods
    {
          ...
          bool visibilidade)
        {
           ...
           .Name == "ExternalDefinitonCreationOptions").ToList();
            if (ls.Count > 0) {
                ...
                value = definitions.GetType().GetMethod("Create", new Type[] { typeof(string), typeof(ParameterType), typeof(bool) }).Invoke(definitions, new object[] { nome, tipo, visibilidade }) as Definition;
            }
            return value;
        }
            ...
            if (met != null) {
              ...
              .GetElement2(levelId), height, offset, flip,
               ...
               .GetType()).ToArray();
                ...
                .GetMethod("NewWall", tipos).Invoke(doc.Create, parametros) as Wall;
            }
            return value;
        }
          ...
          this Element ele, Material m)
        {
            double value = 0;
            Type t = ele.GetType();
            MethodInfo met = t.GetMethod("GetMaterialVolume", new Type[] { typeof(ElementId), typeof(bool) });
            if (met != null) {
                value = (double)met.Invoke(ele, new object[] { m.Id, false });
            } else {
                met = t.GetMethod("GetMaterialVolume", new Type[] { typeof(ElementId) });
          ...
          int espessura = -1)
        {
            ...
            if (ls.Count > 0) {
                ...
                MethodInfo met = view.GetType().GetMethod("set_ProjColorOverrideByElement", new Type[] { typeof(ICollection<ElementId>), typeof(Color) });
                met.Invoke(view, new object[] { ids, cor });
                met = view.GetType().GetMethod("set_ProjLineWeightOverrideByElement", new Type[] { typeof(ICollection<ElementId>), typeof(int) });
