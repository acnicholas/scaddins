namespace SCaddins.ExportSchedules.ViewModels
{
    ...
    public class OptionsViewModel : Screen
    {
        ...
        {
            // Ensure the field delimiter is set to an actual tab character, not the literal string "\t"
            if (string.IsNullOrEmpty(Settings.Default.FieldDelimiter) ||
                Settings.Default.FieldDelimiter == "\\t" ||
                Settings.Default.FieldDelimiter == "/t")
            {
                Settings.Default.FieldDelimiter = "\t"; // Actual tab character
        ...
        public static dynamic DefaultWindowSettings
        {
            get
            {
                ...
                settings.Height = 320;
                settings.Width = 480;
                settings.Title = "nullCarbon - Indstillinger";
