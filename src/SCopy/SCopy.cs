...
public class SCopy
{
    ...
    public enum ViewCreationMode {Copy, CopyAndModify, Replace, Place}
    ...
    {
        foreach (SCopySheet s in sheets) {
        ...
        ref System.Windows.Forms.ListView list)
    {
        ...
        AddViewsToList(ref list,"Title",sourceSheet.Name,colour,0);
        AddViewsToList(ref list,"Sheet Number",sourceSheet.SheetNumber,colour,0);
        AddViewsToList(ref list, sourceSheet.Views);
    }
    ...
    public void CreateSheets()
    {
        ...
        foreach (SCopySheet sheet in sheets) {
            //TaskDialog.Show("debug",sheet.Title);
            CreateSheet(sheet, ref summaryText);
        }
    ...
    public void Add()
    {
        string n = NextSheetNumber(sourceSheet.SheetNumber);
        string t = sourceSheet.Name + SCopyConstants.MenuItemCopy;
        sheets.Add(new SCopySheet(n,t,this));
    }
    ...
    private void GetAllViewsInModel()
    {
        ...
        foreach (View view in c) {
            View v = view as View;
            View vv;
            if(!existingViews.TryGetValue(v.Name,out vv)) {
                existingViews.Add(v.Name, view);
            }
        }
    }
    ...
    private void GetAllLevelsInModel()
    {
        ...
        foreach (Level l in c3) {
            levels.Add(l.Name.ToString(), l);
        }
    }

    private bool CreateSheet(SCopySheet sheet, ref string summary)
    {
        sourceTitleBlock = GetTitleBlock(sourceSheet);
        ...
        ViewSheet destSheet = AddEmptySheetToDocument(
                                  sourceTitleBlock.Symbol, sheet.Number, sheet.Title);
        ...
        var oldNumber = sourceSheet.SheetNumber;
    ...
    {
        FamilyInstance destTB = GetTitleBlock(destSheet);
        ...
        Viewport.Create(doc, destSheet.Id , destViewId, destViewCentre);
    }
    ...
    private string NextSheetNumber(string s)
    {
        ...
        do {
            inc++;
        } while(!CheckSheetNumberAvailability(s + "-" + inc.ToString()));
        ...
        SCopyViewOnSheet view, SCopySheet sheet, XYZ sourceViewCentre)
    {
        ...
        if(level != null) {
            ViewPlan vp = ViewPlan.Create(doc,floorPlanViewFamilyTypeId,level.Id);
            ...
            if( view.ViewTemplateName != SCopyConstants.MenuItemCopy) {
                View vt = null;
                if(viewTemplates.TryGetValue(view.ViewTemplateName, out vt)) {
                    vp.ViewTemplateId = vt.Id;
                }
            }
            PlaceViewOnSheet(sheet.DestSheet, vp.Id, sourceViewCentre);
        }
    }
    ...
    private void PlaceNewViews(SCopySheet sheet)
    {
        ...
        Dictionary<ElementId,BoundingBoxXYZ> viewPorts =
            GetVPDictionary(sourceSheet, doc);
        ...
        foreach(SCopyViewOnSheet view in sheet.ViewsOnSheet) {
            ...
            XYZ sourceViewCentre = ViewCenterFromTBBottomLeft(
                sourceTitleBlock, srcViewBounds, sourceSheet);

            switch (view.CreationMode){
            case ViewCreationMode.Copy:
                CopyViewToSheet(view, sheet, sourceViewCentre);
                break;
            case ViewCreationMode.CopyAndModify:
                PlaceNewView(view, sheet, sourceViewCentre);
                break;     
        ...
        SCopyViewOnSheet view, SCopySheet sheet, XYZ sourceViewCentre)
    {
        ...
        View v = doc.GetElement(destViewId) as View;
        if(newName != null) {
            v.Name = newName;
        }
        PlaceViewOnSheet(sheet.DestSheet, destViewId, sourceViewCentre);
    }
